  ...
  ({
    ...
    useEffect(() => {
      if (wasInitialized) {
        ...
        if (
          typeof initialFromValue === "number" &&
          initialFromValue >= min &&
          initialFromValue <= max
        ) {
          const offset =
            ((initialFromValue - min) / step) * stepSize - knobSize / 2;
          setFromValueStatic(offset, knobSize, stepSize);
          setValueText(offset + knobSize, true);
        }
        if (
          typeof initialToValue === "number" &&
          initialToValue >= min &&
          initialToValue <= max &&
          typeof initialFromValue === "number" &&
          initialToValue > initialFromValue
        ) {
          const offset =
            ((initialToValue - min) / step) * stepSize - knobSize / 2;
    ...
    const setFromValueStatic = (
      newOffset: number,
      knobSize: number,
      stepInPixels: number,
    ) => {
      newOffset =
        Math.floor((newOffset + knobSize / 2) / stepInPixels) * stepInPixels -
        knobSize / 2;
      setFromValue(newOffset);
      setFromValueOffset(newOffset);
      const changeTo =
        Math.floor(
          ((newOffset + knobSize / 2) * (max - min)) / sliderWidth / step,
        ) *
          step +
        min;
    ...
    const setToValueStatic = (
      newOffset: number,
      knobSize: number,
      stepInPixels: number,
    ) => {
      newOffset =
        Math.ceil((newOffset + knobSize / 2) / stepInPixels) * stepInPixels -
        knobSize / 2;
    ...
    const setValueText = (totalOffset: number, from = true) => {
      ...
      if (isFrom || isTo) {
        const numericValue: number =
          Math[isFrom ? "floor" : "ceil"](
            ((totalOffset + knobSize / 2) * (max - min)) / sliderWidth / step,
          ) *
            step +
          min;
    ...
    const onGestureEventFromValue = (event: PanGestureHandlerGestureEvent) => {
      ...
      if (totalOffset >= -knobSize / 2 && totalOffset < toValueOffset) {
        ...
        leftBarScaleX.setValue(
          (totalOffset + knobSize / 2) / sliderWidth + 0.01,
        );
      }
    };
    const onHandlerStateChangeFromValue = (
    ...
    const onGestureEventToValue = (event: PanGestureHandlerGestureEvent) => {
      ...
      if (
        totalOffset <= sliderWidth - knobSize / 2 &&
        totalOffset > fromValueOffset
      ) {
        translateXtoValue.setValue(totalOffset);
        setValueText(totalOffset, false);
        rightBarScaleX.setValue(
          1.01 - (totalOffset + knobSize / 2) / sliderWidth,
        );
      }
    };
    const onHandlerStateChangeToValue = (
      event: PanGestureHandlerGestureEvent,
    ) => {
      if (event.nativeEvent.state === State.BEGAN) {
      ...
      if (event.nativeEvent.state === State.END) {
        ...
        translateXtoValue.setValue(newOffset);
        rightBarScaleX.setValue(
          1.01 - (newOffset + knobSize / 2) / sliderWidth,
        );
    ...
    const scaleTo = (param: Animated.Value, toValue: number) =>
    ...
    const padding = useMemo(
      () => (styleSize === "large" ? 17 : styleSize === "medium" ? 24 : 31),
      [styleSize],
    );
    ...
    return (
      <GestureHandlerRootView>
        <Animated.View
          style={[styles.container, { opacity, padding }, customContainerStyle]}
        >
          {showValueLabels && (
            <View style={{ width: "100%", height: 1, flexDirection }}>
              <KnobBubble
                {...{ knobSize, valueLabelsBackgroundColor }}
